...
(function () {
    ...
    var EditMilestoneCtrl = function ($scope, $rootScope, $state, $stateParams, $log, $window, milestonesFactory, groupsFactory, objectivesFactory, Auth) {
        ...
        function init() {
            ...
            milestonesFactory.getMilestone(milestoneId).error(function (data, status, headers, config) {
                ...
                if (!Auth.isGroupEditor(milestone.groupId)) {
                    $window.alert("\nYou must have Admin or Edit authority for this Group to edit Milestones.\n");
                    $state.go('milestones');
                }
        ...
        $scope.updateMilestone = function () {
            ...
            if (!Auth.isGroupEditor($scope.milestone.groupId)) {
                $window.alert("\nYou must have Admin or Edit authority for this Group to update Milestones.\n");
                $state.go('milestones');
            }
    ...
    EditMilestoneCtrl.$inject = ['$scope', '$rootScope', '$state', '$stateParams', '$log', '$window', 'milestonesFactory', 'groupsFactory', 'objectivesFactory', 'Auth'];
