...
(function () {
    'use strict';
    var GroupOpsStatusReportCtrl = function ($scope, $rootScope, $state, $log, $window, productsFactory, actionsFactory, groupsFactory, usersFactory) {
        ...
        function initializeStatusMatrix() {
            ...
            for (i = 0; i < $scope.products.length; i++) {      /*  process all the products  */
                ...
                for (j = 0; j < $scope.opsGroups.length; j++) { /*  process all the operational groups  */
                    ...
                    matrix[i].groups[j].groupId = $scope.opsGroups[j]._id;
...
//=============================================================================================================
        ...
        function buildStatusMatrix(productStatus) {
            ...
            // Remove all of the groups except for the requested / selected group from all of the matrix entries. Process the group entries backwards.
            
            for (i = 0; i < statusMatrix.length; i++) {
                for (j = statusMatrix[i].groups.length - 1; j >= 0; j--) {
                    if (statusMatrix[i].groups[j].groupId !== $scope.groupId) {
                        statusMatrix[i].groups.splice(j, 1);
                    }
                }
            }
...
//=============================================================================================================
//  opsGroupName:
//      This function returns a group name for the input operational group ID.
//
//  Inputs:             
//      groupId: ObjectId
//
...
//  Process steps:
//      -   loop through $scope.opsGroups array to find the entry that contains the input groupId.
//      -   return the name of the entry with the group ID, or "null" if the group ID was not found.
//
//  Return value(s):    
//        groupName: String (OR null, if group ID not found)
//        
//  Output(s):          
//      none.
//=============================================================================================================

        function opsGroupName(groupId) {
            var i = 0;
            while (i < $scope.opsGroups.length && groupId !== $scope.opsGroups[i]._id) {
                i++;
            }
            if (i < $scope.opsGroups.length) {
                return $scope.opsGroups[i].groupName;
            }
            return null;
        }
...
//=============================================================================================================
        ...
        function processGroupActions(groupActions) {
            ...
            for (i = 0; i < groupActions.length; i++) {
                ...
                if (opsGroupIndex(groupActions[i].groupId) !== null) {
                    ...
                    if (!groupId || groupId !== groupActions[i].groupId) {
                        ...
                        groups[j] = {};
                        groups[j].groupIndex = opsGroupIndex(groupId);
                    }
    ...
    // Authorize this user if logged in email is found in Players.
        ...
        $scope.selectGroup = function () {
            ...
            // Get the actions for the selected Group to populate the $scope.products.groupsStatus matrix

            actionsFactory.getActionsByGroup($scope.groupId).error(function (data, status, headers, config) {
                ...
                $scope.groupName = opsGroupName($scope.groupId);
    ...
    GroupOpsStatusReportCtrl.$inject = ['$scope', '$rootScope', '$state', '$log', '$window', 'productsFactory', 'actionsFactory', 'groupsFactory', 'usersFactory'];

    angular.module('portApp').controller('GroupOpsStatusReportCtrl', GroupOpsStatusReportCtrl);
