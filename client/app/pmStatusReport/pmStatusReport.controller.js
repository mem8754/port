...
(function () {
    'use strict';
    var ProductManagerStatusReportCtrl = function ($scope, $rootScope, $state, $log, $window, productsFactory, actionsFactory, groupsFactory, usersFactory) {
...
//=============================================================================================================
        ...
        function init() {
            ...
            if ($rootScope.userAuthorized) {
                ...
                groupsFactory.getGroups().error(function (data, status, headers, config) {
                    ...
                    usersFactory.getUsersByGroup(pmGroupId).error(function (data, status, headers, config) {
                        $log.warn('Server error getting Product Manager documents: ', status);
                    }).success(function (users) {
                        $scope.users = users.objSort("firstName");
                    });
                });
        ...
        $scope.selectProductManager = function () {

            // Get the actions for the selected Group to populate the $scope.products.groupsStatus matrix

            productsFactory.getProductsByProductManager($scope.productManagerId).error(function (data, status, headers, config) {
    ...
    ProductManagerStatusReportCtrl.$inject = ['$scope', '$rootScope', '$state', '$log', '$window', 'productsFactory', 'actionsFactory', 'groupsFactory', 'usersFactory'];

    angular.module('portApp').controller('ProductManagerStatusReportCtrl', ProductManagerStatusReportCtrl);
